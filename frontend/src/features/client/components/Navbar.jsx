...
const Navbar = () => {
  ...
  const listYourPetPath = loggedIn ? "/vendor/add-pets" : "/login";
  ...
  return (
    <nav className="fixed top-0 left-0 w-full h-16 sm:h-20 flex items-center justify-between px-3 sm:px-6 lg:px-10 bg-white/90 backdrop-blur-md border-b border-gray-100 z-[100] shadow-sm">
      <Link
        to="/"
        className="text-sm sm:text-lg lg:text-xl font-bold text-slate-800 flex items-center gap-1 sm:gap-2 group"
      >
        ...
        <span className="tracking-tight hidden sm:inline text-xs sm:text-sm">
          Shiva Dog Clinic
        </span>
      </Link>
      ...
      <div className="hidden lg:flex items-center space-x-7 h-full">
        <Link
          to="/"
          className={`${isActive("/") ? "text-indigo-600" : "text-slate-600"} hover:text-indigo-600 transition-colors font-bold text-[11px] uppercase tracking-widest`}
        >
          Home
        </Link>
        ...
        {Object.keys(menuData).map((menuName) => (
          ...
          >
            <button
              className={`flex items-center font-bold text-[11px] uppercase tracking-widest transition-colors ${menuData[menuName].some((item) => isActive(item.path)) ? "text-indigo-600" : "text-slate-600 group-hover:text-indigo-600"}`}
            >
              {menuName}{" "}
              <ChevronDown
                size={14}
              ...
              <div className="grid gap-1">
                {menuData[menuName].map((item) => (
                  <Link
                    key={item.name}
                    to={item.path}
                    className={`group/item flex items-center gap-4 p-3 rounded-2xl transition-all ${isActive(item.path) ? "bg-indigo-50" : "hover:bg-slate-50"}`}
                  >
                    <div
                      className={`w-10 h-10 ${item.bg} ${item.color} rounded-xl flex items-center justify-center shrink-0 group-hover/item:animate-bounce shadow-sm`}
                    >
                      {item.icon}
                    </div>
                    <div className="overflow-hidden">
                      <h4
                        className={`text-[13px] font-bold ${isActive(item.path) ? "text-indigo-600" : "text-slate-700"}`}
                      >
                        {item.name}
                      </h4>
        ...
        <Link
          to={listYourPetPath}
          className={`${isActive("/vendor/add-pets") ? "text-indigo-600" : "text-slate-600"} hover:text-indigo-600 transition-colors font-bold text-[11px] uppercase tracking-widest`}
        >
          List Your Pet
        </Link>
        ...
        <Link
          to="/blogs"
          className={`${isActive("/blogs") ? "text-indigo-600" : "text-slate-600"} hover:text-indigo-600 transition-colors font-bold text-[11px] uppercase tracking-widest`}
        >
          Blog
        </Link>
        <Link
          to="/contact"
          className={`${isActive("/contact") ? "text-indigo-600" : "text-slate-600"} hover:text-indigo-600 transition-colors font-bold text-[11px] uppercase tracking-widest`}
        >
          Contact
        </Link>
      </div>
      ...
      <div className="hidden lg:flex items-center space-x-5">
        {loggedIn ? (
          <Link
            to={profilePath}
            className="bg-indigo-600 text-white px-7 py-3 rounded-2xl font-bold hover:bg-indigo-700 shadow-lg shadow-indigo-100 transition-all active:scale-95 text-sm"
          >
            Profile
          </Link>
        ) : (
          <>
            <Link
              to="/login"
              className="text-slate-600 font-bold text-sm hover:text-indigo-600 transition-colors"
            >
              Login
            </Link>
            <Link
        ...
        <div className="absolute top-16 sm:top-20 left-0 w-full bg-white border-b border-gray-100 shadow-lg lg:hidden max-h-[calc(100vh-64px)] sm:max-h-[calc(100vh-80px)] overflow-y-auto">
          <div className="px-3 sm:px-6 py-3 sm:py-4 space-y-2 sm:space-y-3">
            ...
            {Object.keys(menuData).map((menuName) => (
              <div key={menuName} className="border-t border-gray-100 pt-3">
                ...
                >
                  ...
                  <ChevronDown
                    size={16}
                    className={`transition-transform ${expandedMenu === menuName ? "rotate-180" : ""}`}
                  />
                </button>
                ...
                {expandedMenu === menuName && (
                  <div className="space-y-2 pl-4 py-2">
                    {menuData[menuName].map((item) => (
                      ...
                      >
                        <div
                          className={`w-8 h-8 ${item.bg} ${item.color} rounded-lg flex items-center justify-center`}
                        >
                          {item.icon}
                        </div>
            ...
            <Link
              ...
              className={`block px-3 sm:px-4 py-2 sm:py-3 rounded-lg sm:rounded-xl font-bold text-xs sm:text-sm uppercase tracking-widest transition-colors ${isActive("/vendor/add-pets") ? "text-indigo-600 bg-indigo-50" : "text-slate-600 hover:bg-slate-50"}`}
            >
              List Your Pet
            </Link>
