                  ...
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {filteredBookings.map((b) => (
                        <TableRow 
                          key={b.id} 
                          onViewDetails={() => navigate(`/vendor/pet/${b.id}`)} // <-- Passing Navigation Function
                          {...b} 
                        />
                      ))}
                    </tbody>
...
// Updated TableRow component with onViewDetails trigger
const TableRow = ({ img, name, breed, service, sIcon, date, time, status, sColor, onViewDetails }) => (
  <tr className="hover:bg-gray-50/50 transition-colors group">
    <td className="px-8 py-4">
      <div className="flex items-center gap-3">
        <img src={img} className="w-10 h-10 rounded-xl object-cover border border-gray-100 shadow-sm" alt={name} />
